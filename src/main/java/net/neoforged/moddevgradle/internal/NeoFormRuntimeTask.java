...
abstract public class NeoFormRuntimeTask extends DefaultTask {
    ...
    public NeoFormRuntimeTask() {
        ...
        final File gradleHome = project.getGradle().getGradleUserHomeDir();
        final File cacheDir = new File(gradleHome, "caches/neoformruntime");
        ...
        getNeoFormRuntimeLauncher().convention(getJavaToolchainService().launcherFor(new Action<JavaToolchainSpec>() {
            @Override
            public void execute(JavaToolchainSpec spec) {
                spec.getLanguageVersion().set(JavaLanguageVersion.of(21));
            }
        }));
    }
    ...
    protected void run(final List<String> args) {
        ...
        final ArrayList<String> realArgs = new ArrayList<>(args);
        ...
        getExecOperations().javaexec(new Action<JavaExecSpec>() {
            @Override
            public void execute(JavaExecSpec execSpec) {
                ...
                execSpec.executable(NeoFormRuntimeTask.this.getNeoFormRuntimeLauncher().get().getExecutablePath().getAsFile());
                execSpec.classpath(NeoFormRuntimeTask.this.getNeoFormRuntime());
